...
public class PlayerStatisticBarUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        transformImage.sprite = DataManager.Instance.GetSpriteForCurrentTransform(0);
    }
    ...
    private void Player_TryUseSkill(Player.PlayerSkill currentSkill, float manaCost)
    {
        if (currentSkill == Player.PlayerSkill.KamehaSkill
            || currentSkill == Player.PlayerSkill.SpiritBoomSkill
            || currentSkill == Player.PlayerSkill.DragonSkill)
        {
            if (mana.TrySpendAmount(mana.GetMaxMana() / 2))
            {
                Player.Instance.UseSkill();
            }
        } else if (mana.TrySpendAmount(manaCost))
        {
            Player.Instance.UseSkill();
        }
    }
